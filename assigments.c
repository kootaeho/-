...
#include <stdio.h>
#include <string.h>
#include "GPP.h"                           //[평가항목 8] : 다중 소스 파일 사용
...
int main() {
	...
	for (int i = 0; i < 200; i++) {
		...
		//비교 대상 국가
		/*FILE* fp = fopen("C:\\Users\\0307r\\source\\repos\\College.c\\College.c\\countryinfo1.txt", "wt");  //[평가항목 7] : 파일 입출력
		if (fp == NULL) {
			printf("파일 열기 실패");
		}
		...
		fclose(fp);*/
			...
			printf("\n\n");
			FILE* fp = fopen("C:\\Users\\0307r\\source\\repos\\College.c\\College.c\\countryinfo1.txt", "rt"); //[평가항목 7] : 파일 입출력
			printf("-----------------------------------------------------------\n");
